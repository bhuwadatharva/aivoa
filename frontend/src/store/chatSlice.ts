...
const initialState: ChatState = {
  messages: [
    { 
      role: 'assistant', 
      content: "Hello! I am your Aivoa AI CRM Assistant. Speak or type details about your HCP meetings and I will auto-fill your log form. You can also ask me for 'meeting prep briefings' or compliance details." 
    }
  ],
  loading: false,
  error: null
...
export const sendMessage = createAsyncThunk(
  'chat/sendMessage',
  async (chatInput: { message: string; hcp_id?: string | null; context_form_data?: any | null }, { rejectWithValue, dispatch }) => {
    try {
      const response = await axios.post('http://localhost:8000/api/ai/chat', chatInput);
      ...
      return rejectWithValue(err.response?.data?.detail || 'Failed to communicate with AI');
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendMessage.rejected, (state, action) => {
        ...
        state.messages.push({ role: 'assistant', content: `Sorry, I hit an error: ${action.payload}` });
      });
  },
});
